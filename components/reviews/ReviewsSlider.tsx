...
function ReviewCard({ review }: { review: Review }) {
  return (
    <div
      className="group relative flex h-[280px] sm:h-[300px] w-[310px] sm:w-[370px] shrink-0 flex-col justify-between rounded-3xl border border-hs-gold/25 bg-[#072419] p-6 sm:p-7 text-white transition-all duration-300 hover:-translate-y-1.5 hover:border-hs-gold/60"
    >
      <div>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <QuoteIcon className="h-5 w-5 text-hs-gold" />
            <StarRow />
          </div>
          <span className="inline-flex items-center gap-1 rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[0.62rem] font-mono tracking-wider text-hs-cream/70">
        ...
        <p className="mt-4 text-xs sm:text-[0.86rem] leading-relaxed text-hs-cream/90 font-sans line-clamp-4">
      ...
      <div className="border-t border-white/10 pt-4 flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-[0.68rem] text-hs-gold font-mono uppercase tracking-wider">
            {review.title || "Verified Diner"}
          </p>
        </div>

        <span className="text-[0.65rem] font-mono uppercase tracking-widest text-hs-cream/45">
          Bunbury, WA
        </span>
      </div>
    </div>
...
function RatingCard() {
  return (
    <div
      className="relative flex h-[280px] sm:h-[300px] w-[300px] sm:w-[350px] shrink-0 flex-col justify-between rounded-3xl border border-white/15 bg-gradient-to-br from-[#8B3E2F] to-[#6d2f23] p-6 sm:p-7 text-[#FDF8F3] transition-all duration-300 hover:-translate-y-1.5 hover:border-hs-gold/50"
    >
      <div>
        <div className="flex items-center justify-between">
        ...
        <div className="mt-6">
          <span className="font-display text-5xl sm:text-6xl font-bold tracking-tight text-[#FDF8F3]">
            {googleRating.value}
          </span>
          <div className="mt-3">
            <StarRow />
      ...
      <div className="border-t border-white/15 pt-4">
        ...
        <p className="mt-0.5 text-[0.68rem] text-white/70 uppercase tracking-wider font-mono">
...
export function ReviewsSlider() {
  ...
  return (
    ...
    >
      ...
      <div className="reviews-marquee-wrapper relative w-full overflow-hidden py-4 select-none">
        {/* Luxury Edge Gradient Vignettes for Smooth Infinite Fade */}
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-y-0 left-0 z-20 w-16 sm:w-32 bg-gradient-to-r from-hs-cream via-hs-cream/80 to-transparent"
        />
        <div
          aria-hidden="true"
          className="pointer-events-none absolute inset-y-0 right-0 z-20 w-16 sm:w-32 bg-gradient-to-l from-hs-cream via-hs-cream/80 to-transparent"
        />

        {/* Smooth Continuous Moving Track */}
        <div className="reviews-marquee-track flex gap-5 md:gap-6">
          {/* First loop pass */}
