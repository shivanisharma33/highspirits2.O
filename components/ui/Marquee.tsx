    ...
    <ul aria-hidden={hidden || undefined} className="flex shrink-0 items-center">
      {items.map((item) => (
        <li key={item} className="flex items-center whitespace-nowrap">
          <span className="font-display px-8 text-[clamp(1.4rem,2.6vw,2.4rem)] italic tracking-tight md:px-12">{item}</span>
          <span aria-hidden className="h-1.5 w-1.5 rotate-45 bg-hs-gold" />
        </li>
      ))}
    </ul>
  );
